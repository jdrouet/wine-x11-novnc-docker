...
RUN apt-get update \
  && apt-get -y --no-install-recommends install \
    xvfb x11vnc xdotool wget tar supervisor wine32-development net-tools fluxbox \
...
RUN wget -O - https://github.com/novnc/websockify/archive/v0.8.0.tar.gz | tar -xzv -C /root/ \
